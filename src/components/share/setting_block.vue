      ...
      <el-card class="box-card" shadow="alaways" >
        ...
        <el-row>
          ...
          <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="100px" class="demo-ruleForm">
            <el-form-item label="IP地址" prop="ipaddress" v-if="who=='network'">
              <el-input type="text" v-model="ruleForm2.ipaddress" autocomplete="off"></el-input>
            </el-form-item>
            ...
            <el-form-item label="集群名称"  v-if="who=='cluster'">
              <span>{{name}}</span>
            </el-form-item>
            <el-form-item label="集群描述" prop="content" v-if="who=='cluster'">
              <el-input type="text" v-model="ruleForm2.content" auto-complete="offf"></el-input>
            </el-form-item>
            <el-form-item label="主IP"  v-if="who=='cluster'">
              <span>{{cluster_ip}}</span>
            </el-form-item>
            <el-form-item label="已部署节点数量" v-if="who=='cluster'">
              <span>{{node_num}}</span>
            </el-form-item>
      ...
      methods:{
        submitform(formName) {
          ...
          this.$refs[formName].validate(function (valid) {
            if (valid) {
              if (Self.who=='network'){
                Self.$axios.post('',{ip:Self.ruleForm2.ipAddress,gw:Self.ruleForm2.gateway,fdns:Self.ruleForm2.fdns,sdns:Self.ruleForm2.sdns}).then(res=>{
                  console.log(res)
                }).catch(error=>{
                  console.log(error)
                })
                ...
                Self.$axios.post('',{content:Self.ruleForm2.content}).then(res=>{
                  console.log(res)
                }).catch(error=>{
                  console.log(error)
                })
